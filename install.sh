...
openssl ecparam -name secp384r1 -genkey -noout -out ssl/ca.key
openssl req -x509 -new -nodes -key ssl/ca.key -subj "/CN=MyCA" -days 365 -out ssl/ca.crt

#openssl genrsa -out ssl/server.key 2048
openssl ecparam -name secp384r1 -genkey -noout -out ssl/server.key
...
openssl ecparam -name secp384r1 -genkey -noout -out ssl/client.key
