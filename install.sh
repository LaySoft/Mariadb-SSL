...
openssl req -x509 -new -nodes -key ssl/ca.key -subj "/CN=MyCA" -days 999999 -out ssl/ca.crt
...
openssl x509 -req -in ssl/server.csr -CA ssl/ca.crt -CAkey ssl/ca.key -set_serial 100 -out ssl/server.crt -days 999999
...
openssl x509 -req -in ssl/client.csr -CA ssl/ca.crt -CAkey ssl/ca.key -set_serial 101 -out ssl/client.crt -days 999999
